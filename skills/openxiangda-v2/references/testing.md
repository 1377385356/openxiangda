...
用户明确延期的性能项使用报告的 `performanceDeferral`，记录实际授权与后续安排并保留原超标样本。结果明确区分功能通过与性能延期；不补造测量，不放宽功能 AC。
...
修改资源时验证声明、实际交付端的字段语义及受影响的新增、详情、修改、删除、筛选、导出和版本冲突。按实际设计的目标设备验证交互；存在移动办理任务时验证对应移动页面，不受历史后台 PC 约定限制。用允许角色验证成功，用禁止角色验证页面、操作、行和字段边界；存储值及审计应符合声明。平台内部的数据库和性能回归由平台维护者负责，应用不重复搭建平台数据库测试。
