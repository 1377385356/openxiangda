...
上传失败后，本机保留原 OCI 制品，错误的 `details` 包含候选 ID、原 digest、失效时间及脱敏原因码。
恢复网络或登录后，在原工作区执行同一条 `pnpm openxiangda deploy`：相同构建输入、账号、平台、
应用和环境会校验原制品并从平台保存的偏移继续；即使 Docker 暂时不可用也能恢复已经构建的镜像。
