...
| `created_by`/`updated_by` 审计列已支持 `current_user` 行规则（"只看自己创建"）；运行时 WITH CHECK 正向匹配的修复随平台列车交付，使用前确认平台版本 | `{ subject: 'current_user', field: 'created_by', roleCodes: ['app-user'] }` |
| 匿名策略 `requiredFields` 比模型必填更严格会编译报错（标准控件不带必填校验，提交被服务端拒） | 在模型字段上补 `required: true`，与策略对齐 |
