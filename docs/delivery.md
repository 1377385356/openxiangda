...
上传失败后，本机保留原 OCI 制品，错误的 `details` 包含候选 ID、原 digest、失效时间及真实 `causeCode`。
按错误的 `remediation` 处理后，在原工作区执行原 `pnpm openxiangda deploy` 命令（保留原环境等参数）：
相同构建输入、账号、平台、应用、环境和工具版本会校验原制品并从平台保存的偏移继续；即使 Docker
暂时不可用也能恢复已经构建的镜像。
完成响应丢失时同样查询原摘要的 ready 回执。源码（包括未提交文件）、权限主体或目标变化会创建新候选，
不会冒充原上传。上传鉴权仍在平台执行；本地缓存不授予部署权限。

`APPLICATION_IMAGE_STORAGE_BUDGET_EXCEEDED` 表示平台拒绝了存储计费准入，不能靠重新登录解决。
`details.causeDetails` 保留服务器实际提供的应用占用字节、镜像数、应用预算和本次请求字节；未提供
的数据不会推测补齐。请平台核对现役、回滚和运行引用及受管清理结果，确认额度后恢复原上传。
该错误的 `retryable=false` 表示需要先处理额度，上传器不会立即重复请求；原候选仍可在处理后恢复。
正在恢复的候选应先用原工具版本完成上传，再升级 SDK；升级工具版本也会改变候选身份。

缓存保留最多 24 小时、8 个候选、总预算 8 GiB；空间不足时仅回收未被使用的旧候选。
仅当平台单镜像上限不超过 8 GiB 时启用本地恢复，较大的平台配额保留原上传路径，不降低平台允许的镜像大小。
缓存和登录凭据分开，位于本机用户私有目录。过期或已经回收的候选需重新构建；已有 DeploymentRun
的未知结果仍查询原运行，镜像上传恢复不会重新发起已提交的部署。
...
新的发布使用当前源码构建；尚未完成的同输入镜像上传可复用上述短期原制品缓存。
该缓存不保存部署状态，平台仍唯一拥有 DeploymentRun。提交响应不确定时使用原 DeploymentRun 的 status/logs 和 recovery 继续处理。
