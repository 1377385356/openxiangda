...
正式候选参考应用安装在隔离 registry 中失败：openxiangda 的候选隔离规则也阻止了固定 alias 所需的已发布 1.0.268。验收基础设施负责准备这个前置依赖：只从官方 registry 下载根包精确锁定的 V1 版本，同时核对锁文件、metadata 与实际 tarball 的 SHA-512，再用本机 registry 的固定 HTTP 接口装入原始字节。禁止为 openxiangda 开启上游代理，禁止通过 V1 包的 publishConfig 选择写入目的地。七个 V2 候选的冻结清单不包含该辅助依赖。目标限定为 127.0.0.1，网络超时和读取大小有界，任何不一致均在写入前停止；回滚只需删除一次性测试容器和卷。验证覆盖摘要篡改、非精确版本、非本机目标，以及安装后两代引擎各自的版本。

真实 tarball 验收还发现统一 Skill 安装直接调用 core 时没有经过 CLI 的资料目录初始化，错误寻找源码模板。AGENTS 模板的权威位置仍是所选版本根包内的 documentation/AGENTS.md；分发层从 engine.packageRoot 读取该文件，不从全局入口或 core 的开发目录推断。先读模板再开始 Skill 写入，缺少模板即停止；更新继续使用 Skill kit 的托管区块、锁和并发回读。实际 tarball 创建后安装 Skill 并核对项目文字保留，作为正式门禁。
