...
真实 tarball 验收还发现统一 Skill 安装直接调用 core 时没有经过 CLI 的资料目录初始化，错误寻找源码模板。AGENTS 模板的权威位置仍是所选版本根包内的 docs/AGENTS.md；分发层从 engine.packageRoot 读取该文件，不从全局入口或 core 的开发目录推断。先读模板再开始 Skill 写入，缺少模板即停止；更新继续使用 Skill kit 的托管区块、锁和并发回读。实际 tarball 创建后安装 Skill 并核对项目文字保留，作为正式门禁。
