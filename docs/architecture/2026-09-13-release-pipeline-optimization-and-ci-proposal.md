...
## 八、无声死亡根因定位与设计审查（2026-09-14）

### 8.1 根因定位（证据链）

三次 CI 无声死亡的时间戳取证呈同一模式：

| 运行 | 模式 | 步骤时长 | 最后日志 → 结论 | 静默尾巴 |
| --- | --- | --- | --- | --- |
| 34774181478 | 串行 | 14m18s | 18:27:43 → 18:34:34 | ~7 分钟 |
| 34775465070 | 并行 | 10m05s | 18:47:43 → 18:54:51 | ~7 分钟 |
| 34777270717 | 串行（npm 已写入） | 19m36s | 19:27:14 → 19:40:15 | ~13 分钟 |

**分层根因**：

1. **表现层（已修复）**：`runReleaseCommand` 以 `detached` 进程组派生子进程（为了超时能整组杀），但主进程死亡/失败路径从不清理其余进程组。孤儿（vite/esbuild/playwright webserver）持有 stdout 管道，Actions runner 等管道 EOF 直到超时——这就是"主进程瞬时死亡后 7-13 分钟静默、无错误行、步骤失败"的签名。本地亦有直接证据：并行验证失败后日志持续被孤儿 webserver 写入 `[WebServer] ECONNREFUSED`。
2. **触发层（高置信推断，待心跳数据确认）**：主进程被宿主无声杀死（SIGKILL 无输出）。运行 #3 在静默尾巴里完成了 npm 五包写入+dist-tag，说明死亡发生在发布中后段；前两次死于验证中段（工作区测试黑盒阶段——内存峰值最高的阶段：Nest 应用+dev 服务器+构建并发）。runner 前置快照显示 15GB 可用，但峰值未被观测——心跳监控已加入（30 秒间隔记录 RSS/进程/磁盘，随 release-state artifact 留存），下次再发生即有第一现场。
3. **已修复的遏制**：任一门禁失败→立即杀死全部兄弟进程组；进程退出钩子兜底 SIGKILL。自测通过（2 秒失败→孤儿全灭→干净退出）。发布通道不再出现"静默挂起"表现。

### 8.2 设计审查：对照业界标准

对照 changesets/action 官方流、GitHub 官方 Node 发布指南、semantic-release/lerna 通用实践：

**设计合理、保留的部分**

| 设计 | 业界对照 | 结论 |
| --- | --- | --- |
| OIDC trusted publishing + environment 审批 | npm/GitHub 官方推荐 | ✓ 已达标 |
| changesets 确定性版本 + Version PR | 标准 | ✓ 已建（试点后开自动） |
| 发布字节=验证字节、npm 不可变纪律 | 优于多数开源项目 | ✓ 保留 |
| 按字节差异的增量门禁 | 类似 turbo affected 思想 | ✓ 保留（发布线/路径投影缺陷已修） |
| registry 读回 + prefer-online | 发布安全加固 | ✓ 保留 |

**真正偏离业界、造成实际事故的设计缺陷**

| # | 缺陷 | 业界做法 | 已造成的事故 | 改造 |
| --- | --- | --- | --- | --- |
| D1 | **单一巨型验证进程**：一个 node 进程串/并管 10+ 阶段、内部预算状态机 | 每类门禁一个 workflow job：独立超时、独立日志、独立重试；job 失败 Actions 杀整棵进程树 | 无声死亡难归因；本地/CI 表现不一致 | 验证拆为 workflow jobs（workspace / fresh-app / reference 三腿并行 job），`needs` 汇聚 |
| D2 | **发布机器即打包机器**：回执/候选 tarball 绑定在发布进程的 `.git`，跨机器 tarball 字节不同（gzip 平台差异） | **build once, publish the artifact**：构建 job 打包→upload-artifact→发布 job 下载校验 integrity 后发布 | 2.18.2 的"npm 已发、收尾死锁、本地无法恢复" | 制品交接：build job 冻结候选→artifact；publish job 下载+校验+发布。副产品：发布 job 降到 ~5 分钟 |
| D3 | **回执存在 `.git` 单机目录** | 状态在 workflow/GitHub Release/artifact | runner 消失=恢复链断裂 | 随 D2 落地（release-state artifact 已加，接收端待接） |
| D4 | **外部参考仓作为门禁前置**（需第二仓写凭据+推送时序） | 参考应用在仓内同 job 验证 | 参考锁过期反复返工；CI 需额外凭据 | 中期：打包分发已在生成独立验收应用，参考仓降级为下游同步（非门禁） |
