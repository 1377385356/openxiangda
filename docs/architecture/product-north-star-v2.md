...
2026-09-11 用户确认采用 OpenDesign 完善整体设计，并允许移除冲突的旧设计理念。界面任务默认先形成具体视觉与可运行原型，再实现与验证；旧“统一默认外观”及后台一律 PC 的要求已被此决定取代。设备范围按任务确定，PC/移动字段行为继续复用，视觉可优化。平台保持路由、授权和数据所有权；见[OpenDesign 接入决定](./2026-09-11-opendesign-capabilities.md)。
