...
待办、通知、文件和聚合。Nest 是复杂业务、集成与后台任务的可选扩展。

2026-09-17 用户根据实际效果确认移除 OpenDesign：它不再是界面任务的运行时或中间层，`design` CLI、原版 MCP/桌面桥接与专属资料一并删除。界面任务由当前 AI Agent 从 AppSpec 和平台契约出发，按需使用 Image 2.5 等图片生成能力形成少量视觉参考，直接实现真实 React 页面并通过浏览器迭代。图片不拥有结构、交互、权限或验收事实；AppSpec、平台 Shell、PC/移动字段行为、路由、授权和数据所有权保持不变。见[Agent 原生视觉开发决定](./2026-09-17-agent-native-visual-development.md)。
