...
- 界面工作默认采用 docs/design-workflow.md 的 Agent 原生闭环：按需用 Image 2.5 等当前图片能力生成少量视觉参考，直接实现真实页面并在浏览器修正。图片不拥有交互、权限或验收事实；主题由应用显式传入，不新增全局偏好状态，Tailwind 不向共享运行时引入 preflight。
- 设备范围由真实任务决定，不以历史 PC 限制否决必要移动设计。标准 CRUD 与自定义后台报表/工具共用平台路由和显式菜单；专业交互主动评估成熟组件，图表优先评估 ECharts。
