...
export const MCP_TOOL_DEFINITIONS = {
  ...
  appspec_verify: definition('核对业务验收报告', '读取成功测试运行与 Git 中的真实验收报告，核对版本绑定、场景结果和性能证据；不自动编造或执行验收。', z.object({ deploymentId: z.string().min(1), evidencePath: z.string().optional().describe('appspec/verification/*.json；省略时按运行 ID 读取') })),
